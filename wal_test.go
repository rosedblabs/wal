...
func TestWAL_ReaderWithStart(t *testing.T) {
	...
	for _, tt := range tests {
		...
		testWriteAndIterate(t, wal, 500000, 512)
		...
		reader3, err := wal.NewReaderWithStart(&ChunkPosition{SegmentId: 1, BlockNumber: 5, ChunkOffset: 0})
		assert.Nil(t, err)
		_, pos3, err := reader3.Next()
		assert.Nil(t, err)
		assert.Equal(t, pos3.SegmentId, uint32(1))
